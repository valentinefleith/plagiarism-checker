...
def la_pipeline() -> Pipeline:
    ...
    return Pipeline(
        [
            ...
            ),  ## utilisation de bi-gramme ça donne plus de contexte je pense c'est mieux que juste des ngramme
...
def evaluer_modele(X_train, y_train, X_test, y_test) -> float:
    ...
    print(
        f"Accuracy: {accuracy:.4f}, Précision: {precision:.4f}, Rappel: {recall:.4f}, F-mesure: {f1:.4f}"
    )
    ...
    sns.heatmap(
        ...
        fmt="d",
        cmap="Blues",
        xticklabels=["Humain", "LLM"],
        yticklabels=["Humain", "LLM"],
    )
...
def main():
    ...
    )  ## on appelle la fonction pour evaluer notre modele
